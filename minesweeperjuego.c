...
int juego(int difc) {
		...
		int col, row, min, x, y, prueba, prueba2;
		...
		tipoCasilla **red = crearMatrix(col, row); // Red de 16x16, 40 minas
		inicializarMatriz(red, col, row);
		colocarMinas(col, row, red, 40);
		print_matrix(col, row, red, difc);
		scanf("%d%d \n", &x, &y);
